...
export const api = {
  ...
  login: (username, password) => {
    ...
    return fetch(`${BASE}/auth/login`, { method: "POST", body }).then(
      async (r) => {
        if (!r.ok) {
  ...
  createDemand: (data) =>
    request("/demands", { method: "POST", body: JSON.stringify(data) }),
  submitDemand: (id) => request(`/demands/${id}/submit`, { method: "POST" }),
  forwardDemand: (id, dc_id) =>
    request(`/demands/${id}/forward`, {
      method: "POST",
      body: JSON.stringify({ dc_id }),
    }),
  approveDemand: (id, data) =>
    request(`/demands/${id}/approve`, {
      method: "POST",
  ...
  createIssueOrder: (data) =>
    request("/issue-orders", { method: "POST", body: JSON.stringify(data) }),

  // Gate Passes
  getGatePasses: () => request("/gate-passes"),
  getGatePass: (id) => request(`/gate-passes/${id}`),
  createGatePass: (data) =>
    request("/gate-passes", { method: "POST", body: JSON.stringify(data) }),
  withdrawItems: (id) =>
    request(`/gate-passes/${id}/withdraw`, { method: "POST" }),
